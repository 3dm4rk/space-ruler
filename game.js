...
// =========================
// 🔄 PVP ORIENTATION GUARD (mobile)
// - On battle.html (PVP), if device is in portrait on a small screen,
//   show a full-screen overlay asking the user to rotate to landscape.
// - In landscape, hide overlay so gameplay is full-width.
// =========================
function setupPvpOrientationGuard(){
  try{
    const b = document.body;
    if (!b) return;
    const isPvpPage = b.classList.contains("pvpPage") || !!window.__FORCE_PVP__;
    if (!isPvpPage) return;

    const overlay = document.getElementById("rotateOverlay");

    const apply = ()=>{
      const small = window.matchMedia("(max-width: 980px)").matches || window.matchMedia("(max-height: 600px)").matches;
      const portrait = window.matchMedia("(orientation: portrait)").matches;

      b.classList.toggle("isPortrait", !!portrait);
      b.classList.toggle("isLandscape", !portrait);

      // CSS handles display, but keep a JS fallback in case CSS doesn't match (older browsers)
      if (overlay){
        overlay.style.display = (small && portrait) ? "flex" : "none";
      }

      // Prevent background scroll when blocking
      const block = small && portrait;
      document.documentElement.style.overflow = block ? "hidden" : "";
      b.style.overflow = block ? "hidden" : "";
    };
...
function showReadyModal(onReady){
  ...
  window.addEventListener("DOMContentLoaded", ()=>{
    ...
    // PVP portrait guard (mobile)
    try{ setupPvpOrientationGuard(); }catch(e){}
